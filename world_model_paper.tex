...
Dependent type theory gives that picture real discipline. A state may carry the
evidence that justifies it, and an action may be typed by the state that
authorizes it. Rocq gives us a concrete language for this line of research.
...
kept compact, but the claim is ambitious: this is a promising path for making
world models more explicit about what they know, what they can combine, and what
they are entitled to do next.
...
Figure~\ref{fig:latent-decision} is the picture behind the paper: the latent
space is a product of coordinates, the join is an intersection of compatible
slices, and admissibility is the witness that the intersection is safe to act on.
...
\centering
\small
\textbf{Join in $n$-Dimensional Latent Space}\\[0.6em]
\[
  \mathcal{Z} = Z_1 \times Z_2 \times \cdots \times Z_n
  \qquad
  z = (z_1, z_2, \ldots, z_n)
\]
\[
  \mathcal{S}_i = \{ z \in \mathcal{Z} : \mathsf{Formed}(e_i,z_i) \}
  \qquad
  \mathcal{J} = \bigcap_{i=1}^{m} \pi_i^{-1}(\mathcal{S}_i)
\]
\[
  z^{\star} \in \mathcal{J}
  ...
  \mathsf{Adm}(z^{\star})
  \qquad\Longrightarrow\qquad
  a \in \mathsf{Next}(z^{\star},\mathsf{Adm}(z^{\star}))
\]
\[
  \mathcal{A} = \{ z^{\star} \in \mathcal{J} : \mathsf{Adm}(z^{\star}) \}
\]
\end{minipage}%
}
\caption{A compact picture of the paper's claim: an admissible decision arises
only after many latent coordinates are joined into a compatible region of the
full latent space.}
