...
Let $\mathsf{Compat} : E \times E \to \mathsf{Prop}$ express evidence
compatibility, let $\oplus_Z : Z \times Z \to Z$ combine latent summaries, and
let $\oplus_E : E \times E \to E$ combine evidence bundles. Suppose further
that admissibility claims are represented by a predicate
$\mathsf{Claim} : E \times Z \to \mathsf{Prop}$. If
\[
  h_1 : \mathsf{Formed}(e_1,z_1), \qquad
  h_2 : \mathsf{Formed}(e_2,z_2), \qquad
  k : \mathsf{Compat}(e_1,e_2),
\]
then a composite checked latent may be formed as
\[
  (z_1,e_1,h_1) \otimes (z_2,e_2,h_2)
  = (z_1 \oplus_Z z_2,\, e_1 \oplus_E e_2,\, h_{12}),
\]
for some witness
$h_{12} : \mathsf{Formed}(e_1 \oplus_E e_2,\, z_1 \oplus_Z z_2)$, and any
admissibility claim must be stated over the composite pair
$(e_1 \oplus_E e_2, z_1 \oplus_Z z_2)$ rather than over the latent alone.
\end{proposition}

The point of the proposition is not algebraic flourish. It is to insist that
when several latent fragments are merged, the right object of trust is the
joined evidence-latent pair. A downstream planner should not inherit a stronger
claim merely because it has access to more vectors. It should inherit only those
claims supported by a lawful composition of the evidence behind them.
